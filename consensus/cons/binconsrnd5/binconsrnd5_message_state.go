...
func (sms *MessageState) getValidsAuxProof(round types.ConsensusRound, mc *consinterface.MemCheckers) (
	...
	case 3:
		round1Struct := sms.getAuxRoundStruct(1, mc)
		round2Struct := sms.getAuxRoundStruct(2, mc)
		if round2Struct.TotalBinAuxMsgCount < nmt { // be sure we have enough messages
			return [2]bool{false, false}, false
		}
		return [2]bool{round1Struct.BinNumsAux[0] >= nmt, round2Struct.BinNumsAux[1] >= nmt}, false
	default:
		prvRoundStruct := sms.getAuxRoundStruct(round-1, mc)
...
func (sms *MessageState) getValidsAuxBoth(round types.ConsensusRound, mc *consinterface.MemCheckers) [3]bool {
	...
	case 3:
		// either n-t for each bin value from aux in round 3
		// or got n-t 0 in round 1, and n-t 1 in round 2
		round2Struct := prvRoundStruct
		round1Struct := sms.getAuxRoundStruct(1, mc)
		return [3]bool{roundStruct.BinNumsAux[0] >= nmt, roundStruct.BinNumsAux[1] >= nmt,
			round1Struct.gotCoin && round1Struct.BinNumsAux[0] >= nmt && round2Struct.BinNumsAux[1] >= nmt &&
...
func (sms *MessageState) New(idx types.ConsensusIndex) consinterface.MessageState {
	...
	var maxPresetRound types.ConsensusRound = 2
	myPresets := []struct {
...
func (sms *MessageState) GotMsg(hdrFunc consinterface.HeaderFunc,
	...
	// Check if the message is a valid format (before the signature is validated)
	switch w := deser.Header.(messages.InternalSignedMsgHeader).GetBaseMsgHeader().(type) {
	case *messagetypes.AuxProofMessage:
		if w.BinVal == types.Coin && (!gc.AllowSupportCoin || w.Round <= sms.maxPresetCoinRound) {
	...
	case *messagetypes.AuxProofMessage:
		...
		default:
			...
			if w.Round > 2 && len(roundStruct.auxBothHeaders) == 0 && foundVal >= 0 {
...
func (sms *MessageState) GenerateProofs(headerID messages.HeaderID, sigCount int, round types.ConsensusRound,
	...
	//    Round 2: (i) n-t auxProof round 1 messages for 0 (ii) t+1 auxProof round 0 messages for 1
	//    Round 3: (i) n-t auxProof round 1 messages for 0 (ii) n-t auxProof round 2 messages for 1
	//    Round r > D32: (i) n-t auxBoth messages for bot from previous round -> This also needs to be a valid coin value
	...
	case 3:
		...
		case messages.HdrAuxProof:
			...
			case 0: // we need n-t 0 messages from round 1
				prfRound = 1
				count = nmt
			case 1: // we need n-t messages from round 2
				prfRound = 2
				count = nmt
			default:
				panic(binVal)
		...
		case messages.HdrAuxBoth:
			if binVal <= 1 {
				...
				// We also need n-t round 2 messages for 1
				sm2, err := sms.GetProofs(messages.HdrAuxProof, nmt, 2, 1, pub, mc)
				if err != nil {
					return sm2, err
				}
				if sm2[0].GetSigCount() < nmt {
