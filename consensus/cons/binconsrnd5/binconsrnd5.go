...
func (sc *BinConsRnd5) NeedsCompletionConcurrentProposals() types.ConsensusInt {
	return 1
}
...
func (sc *BinConsRnd5) SetInitialState([]byte, storage.StoreInterface) {}
...
func (sc *BinConsRnd5) CheckRound(nmt int, t int, round types.ConsensusRound,
	...
	if auxProofValids[1] {
		// check broadcast 1
		if sc.AllowSupportCoin && round > binMsgState.maxPresetCoinRound {
	...
	if round == 1 {
		...
		if roundStruct.BinNumsAux[0] >= nmt {
			shouldDecide = 0
		}
	} else {
...
func (sc *BinConsRnd5) checkBroadcastsAuxCoin(est types.BinVal, supportCoin bool,
	...
	if !roundStruct.sentAuxProof && sc.CheckMemberLocal() {
		...
		if sc.CheckMemberLocalMsg(auxMsg) {
			if round > 2 && !(round == 2 && sc.getMsgState().isMv) { // don't use a coin for round 1 of multi value reduction
...
func (sc *BinConsRnd5) checkBroadcastsAux(est types.BinVal, supportCoin bool, round types.ConsensusRound,
	...
	if (round == 1 || round == 2) && supportCoin {
		panic("cannot support coin on round 1 or 2")
	}
...
func (sc *BinConsRnd5) GetDecision() (sig.Pub, []byte, types.ConsensusIndex, types.ConsensusIndex) {
	if sc.Decided == -1 {
