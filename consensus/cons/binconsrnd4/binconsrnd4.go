...
func (sc *BinConsRnd4) getMsgState() *MessageState { // TODO clean this up
...
func (sc *BinConsRnd4) GetCommitProof() []messages.MsgHeader {
	return bincons1.GetBinConsCommitProof(messages.HdrAuxStage1, sc, sc.getMsgState(), true, sc.ConsItems.MC)
}
...
func (sc *BinConsRnd4) GetBinState(localOnly bool) ([]byte, error) {
...
func (sc *BinConsRnd4) GetProposeHeaderID() messages.HeaderID {
	return messages.HdrBinPropose
}

// GenerateNewItem creates a new bin cons item.
func (*BinConsRnd4) GenerateNewItem(index types.ConsensusIndex,
	...
	newItem := &BinConsRnd4{AbsConsItem: newAbsItem}
...
func (sc *BinConsRnd4) GetProposalIndex() (prevIdx types.ConsensusIndex, ready bool) {
	if sc.NeedsProposal {
...
func (sc *BinConsRnd4) NeedsConcurrent() types.ConsensusInt {
	return 1
}

// GetBinDecided returns -1 if not decided, or the decided value and the decided round.
func (sc *BinConsRnd4) GetBinDecided() (int, types.ConsensusRound) {
...
func (sc *BinConsRnd4) ProcessMessage(
	...
	if (sc.terminated && round > sc.terminatedRound) ||
		(sc.StopOnCommit == types.NextRound && sc.Decided > -1 && round > sc.decidedRound+1) {

		logging.Infof("Got a msg for round %v, but already decided in round %v", round, sc.decidedRound)
		return false, false
	}

	// check if we can now advance rounds (note the message was already stored to the bincons state in BinConsRnd4MessageState.GotMsg
...
func (sc *BinConsRnd4) CanSkipMvTimeout() bool {
	return sc.getMsgState().getAuxRoundStruct(2, sc.ConsItems.MC).TotalAuxBinMsgCount >
		sc.ConsItems.MC.MC.GetMemberCount()-sc.ConsItems.MC.MC.GetFaultCount()
}

// SetInitialState does noting for this algorithm.
func (sc *BinConsRnd4) SetInitialState([]byte) {}
...
func (sc *BinConsRnd4) CheckRound(nmt int, t int, round types.ConsensusRound,
...
func (sc *BinConsRnd4) checkDone(round types.ConsensusRound, nmt, t int) bool {
	_ = t
	if sc.terminated && round >= sc.terminatedRound {
		return true
	}
	...
	case types.SendProof:
		if !sc.NoSignatures && !sc.terminated {
			...
			prfMsgs := bincons1.GetBinConsCommitProof(messages.HdrAuxStage1, sc, sc.getMsgState(), false, sc.ConsItems.MC)
...
func (sc *BinConsRnd4) broadcastMsg(msgType messages.HeaderID, binVal types.BinVal, round types.ConsensusRound, stage byte) {
...
func (sc *BinConsRnd4) GetBufferCount(hdr messages.MsgIDHeader, gc *generalconfig.GeneralConfig, memberChecker *consinterface.MemCheckers) (int, int, messages.MsgID, error) {
	switch hdr.GetID() {
...
func (sc *BinConsRnd4) GetHeader(emptyPub sig.Pub, gc *generalconfig.GeneralConfig, headerType messages.HeaderID) (messages.MsgHeader, error) {
	var msg messages.InternalSignedMsgHeader
	switch headerType {
...
func (sc *BinConsRnd4) GetNextInfo() (prevIdx types.ConsensusIndex, proposer sig.Pub, preDecision []byte, hasNextInfo bool) {
...
func (sc *BinConsRnd4) GetDecision() (sig.Pub, []byte, types.ConsensusIndex) {
	if sc.Decided == -1 {
...
func (sc *BinConsRnd4) GetConsType() types.ConsType {
	return types.BinConsRnd4Type
}

// PrevHasBeenReset is called when the previous consensus index has been reset to a new index
func (sc *BinConsRnd4) PrevHasBeenReset() {
}
...
func (sc *BinConsRnd4) SetNextConsItem(consinterface.ConsItem) {
}

// ShouldCreatePartial returns true if the message type should be sent as a partial message
func (sc *BinConsRnd4) ShouldCreatePartial(messages.HeaderID) bool {
	return false
}
...
func (*BinConsRnd4) GenerateMessageState(gc *generalconfig.GeneralConfig) consinterface.MessageState {

	return NewBinConsRnd4MessageState(false, gc)
}

// Collect is called when the item is being garbage collected.
func (sc *BinConsRnd4) Collect() {
}
