...
// BinConsRnd3MessageState implements BinConsMessageStateInterface.
// It stores the messages of BinConsRnd3.
...
func (sms *MessageState) getValidsAuxBoth(round types.ConsensusRound, mc *consinterface.MemCheckers) [3]bool {
	...
	switch round {
	case 0:
		panic("aux both not supported in round 0")
	case 1:
		// either n-t for each bin value from aux in round 1
		// or got t+1 for both values in round 0
		return [3]bool{roundStruct.BinNumsAux[0] >= nmt, roundStruct.BinNumsAux[1] >= nmt,
			prvRoundStruct.gotCoin && prvRoundStruct.BinNumsAux[0] > t && prvRoundStruct.BinNumsAux[1] > t &&
...
// NewBinConsRnd3MessageState generates a new BinConsRnd3MessageState object.
func NewBinConsRnd3MessageState(isMv bool,
...
// New creates a new empty BinConsRnd3MessageState object for the consensus index idx.
...
// The message must be an AuxProofMessage, since that is the only valid message type for BinConsRnd3.
func (sms *MessageState) GotMsg(hdrFunc consinterface.HeaderFunc,
	...
	case *messagetypes.AuxProofMessage:
		if w.BinVal == types.Coin && (!gc.AllowSupportCoin || w.Round <= 1) {
...
func (sms *MessageState) GenerateProofs(headerID messages.HeaderID, sigCount int, round types.ConsensusRound,
	...
	//    Round r > 1: (i) n-t auxBoth messages for bot from previous round -> This also needs to be a valid coin value
	//                 (ii) n-t auxProof messages for v from previous round
	// For both message ->
	//    To support bin: n-t auxProof messages from same round
	//    To support bot:
	//                Round r = 1: t+1 auxProof messages for both 0 and 1
	...
	case 1:
		...
		case messages.HdrAuxBoth:
			if binVal <= 1 {
				// need n-t round 1 auxProof messages
				sm, err := sms.GetProofs(messages.HdrAuxProof, sigCount, 1, binVal, pub, mc)
				if err != nil {
					return sm, err
				}
				...
				// need t+1 round 0 messages for both 0 and 1
				sm, err := sms.GetProofs(messages.HdrAuxProof, t+1, 0, 0, pub, mc)
				if err != nil {
					return sm, err
				}
				if sm[0].GetSigCount() < mc.MC.GetFaultCount() {
