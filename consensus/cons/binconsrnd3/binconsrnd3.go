...
func (sc *BinConsRnd3) getMsgState() *MessageState { // TODO clean this up
...
func (sc *BinConsRnd3) GetCommitProof() []messages.MsgHeader {
	return bincons1.GetBinConsCommitProof(messages.HdrAuxBoth, sc, sc.getMsgState(), true, sc.ConsItems.MC)
}
...
func (sc *BinConsRnd3) GetBinState(localOnly bool) ([]byte, error) {
...
func (sc *BinConsRnd3) GetProposeHeaderID() messages.HeaderID {
	return messages.HdrBinPropose
}

// GenerateNewItem creates a new bin cons item.
func (*BinConsRnd3) GenerateNewItem(index types.ConsensusIndex,
	...
	newItem := &BinConsRnd3{AbsConsItem: newAbsItem}
...
func (sc *BinConsRnd3) Start(finishedLastRound bool) {
	_ = finishedLastRound
...
func (sc *BinConsRnd3) GetMVInitialRoundBroadcast(types.BinVal) messages.InternalSignedMsgHeader {
	panic("TODO")
}
...
func (sc *BinConsRnd3) GotProposal(hdr messages.MsgHeader, mainChannel channelinterface.MainChannel) error {
	sc.AbsGotProposal()
...
func (sc *BinConsRnd3) NeedsCompletionConcurrentProposals() types.ConsensusInt {
	return 1
}

// GetBinDecided returns -1 if not decided, or the decided value and the decided round.
func (sc *BinConsRnd3) GetBinDecided() (int, types.ConsensusRound) {
...
func (sc *BinConsRnd3) ProcessMessage(
	...
	// check if we can now advance rounds (note the message was already stored to the bincons state in BinConsRnd3MessageState.GotMsg
...
func (sc *BinConsRnd3) CanSkipMvTimeout() bool {
	panic("TODO")
...
func (sc *BinConsRnd3) SetInitialState([]byte, storage.StoreInterface) {}
...
func (sc *BinConsRnd3) CheckRound(nmt int, t int, round types.ConsensusRound,
	...
	// Check if we have enough valid messages to send an auxBoth message
	var totalValids int
	for i := 0; i < 2; i++ {
		if auxProofValids[i] {
			totalValids += roundStruct.BinNumsAux[i]
		}
	}
	if totalValids >= nmt && roundStruct.TotalBinAuxMsgCount >= nmt {
	...
	if shouldDecide < 2 {
		if sc.Decided >= 0 {
			...
			// fmt.Printf("Decided proc %v bin round %v, binval %v, index %v\n", sc.TestIndex, round, shouldDecide, sc.Index)
...
func (sc *BinConsRnd3) checkDone(round types.ConsensusRound, nmt, t int) bool {
	_ = nmt
	if sc.terminated {
		return true
	}
	...
	case types.SendProof:
		...
		prfMsgs := bincons1.GetBinConsCommitProof(messages.HdrAuxBoth, sc, sc.getMsgState(), false, sc.ConsItems.MC)
...
func (sc *BinConsRnd3) beforeBcastSanityCheck(
	est types.BinVal,
...
func (sc *BinConsRnd3) checkBroadcastsAuxCoin(est types.BinVal, supportCoin bool,
	round types.ConsensusRound,
	binMsgState *MessageState,
	mainChannel channelinterface.MainChannel) bool {
...
func (sc *BinConsRnd3) checkBroadcastsAux(est types.BinVal, supportCoin bool, round types.ConsensusRound,
	binMsgState *MessageState,
	mainChannel channelinterface.MainChannel) bool {
...
func (sc *BinConsRnd3) checkBroadcastsAuxBoth(est types.BinVal, round types.ConsensusRound,
	binMsgState *MessageState,
	mainChannel channelinterface.MainChannel) bool {
...
func (sc *BinConsRnd3) checkBroadcastsCoin(round types.ConsensusRound,
	binMsgState *MessageState,
	mainChannel channelinterface.MainChannel) bool {
...
func (sc *BinConsRnd3) generateProofsInternal(hdr messages.MsgIDHeader, supportCoin bool, est types.BinVal,
...
func (sc *BinConsRnd3) GetBufferCount(hdr messages.MsgIDHeader, gc *generalconfig.GeneralConfig, memberChecker *consinterface.MemCheckers) (int, int, messages.MsgID, error) {
	switch hdr.GetID() {
...
func (sc *BinConsRnd3) GetHeader(emptyPub sig.Pub, gc *generalconfig.GeneralConfig, headerType messages.HeaderID) (messages.MsgHeader, error) {
	switch headerType {
...
func (sc *BinConsRnd3) GetNextInfo() (prevIdx types.ConsensusIndex, proposer sig.Pub, preDecision []byte, hasNextInfo bool) {
...
func (sc *BinConsRnd3) GetDecision() (sig.Pub, []byte, types.ConsensusIndex, types.ConsensusIndex) {
	if sc.Decided == -1 {
...
func (sc *BinConsRnd3) GetConsType() types.ConsType {
	return types.BinConsRnd3Type
}

// PrevHasBeenReset is called when the previous consensus index has been reset to a new index
func (sc *BinConsRnd3) PrevHasBeenReset() {
}
...
func (sc *BinConsRnd3) SetNextConsItem(next consinterface.ConsItem) {
	_ = next
}

// ShouldCreatePartial returns true if the message type should be sent as a partial message
func (sc *BinConsRnd3) ShouldCreatePartial(headerType messages.HeaderID) bool {
	_ = headerType
	return false
}
...
func (sc *BinConsRnd3) BroadcastCoin(coinMsg messages.MsgHeader,
...
func (*BinConsRnd3) GenerateMessageState(gc *generalconfig.GeneralConfig) consinterface.MessageState {

	return NewBinConsRnd3MessageState(false, gc)
}
