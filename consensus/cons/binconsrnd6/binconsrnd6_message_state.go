...
// BinConsRnd6MessageState implements BinConsMessageStateInterface.
// It stores the messages of BinConsRnd6.
...
// NewBinConsRnd6MessageState generates a new BinConsRnd6MessageState object.
func NewBinConsRnd6MessageState(isMv bool,
...
// New creates a new empty BinConsRnd6MessageState object for the consensus index idx.
...
func (sms *MessageState) GetProofs(headerID messages.HeaderID, sigCount int, round types.ConsensusRound, binVal types.BinVal,
	...
	if headerID != messages.HdrAuxStage1 && headerID != messages.HdrAuxProof {
...
func (sms *MessageState) getAuxRoundStruct(round types.ConsensusRound,
	...
	if item == nil {
		...
		case 1:
			...
			item.sentCoin = true
		case 2:
			rnd1 := sms.getAuxRoundStruct(1, mc)
			item.supportBvInfoStage0[0] = &item.bvInfoStage0[0]
			item.supportBvInfoStage0[1] = &rnd1.bvInfoStage0[1]
			item.gotCoin = true
			item.coinVal = 0
			item.sentCoin = true
		case 3:
			rnd2 := sms.getAuxRoundStruct(2, mc)
			item.supportBvInfoStage0[1] = &item.bvInfoStage0[1]
			item.supportBvInfoStage0[0] = &rnd2.bvInfoStage0[0]
		}

		if round == 1 && sms.isMv { // when using mv reduction, first round coin value is always 1 so we can decide 1 right away
...
// The message must be an AuxProofMessage, since that is the only valid message type for BinConsRnd6.
