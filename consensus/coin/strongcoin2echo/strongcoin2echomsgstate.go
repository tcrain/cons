...
// StrongCoin2EchoMsgState represents the state of messages implementing a strong coin
...
func (sms *MsgState) GotMsg(msgState consinterface.MessageState, deser *channelinterface.DeserializedItem,
	...
	case *messagetypes.CoinMessage:
		...
		checkCoin:
			...
			coinVal, ready, err := msgState.GetCoinVal(w, coinThrsh, mc)
			switch err {
			case nil:
				if ready {
					// sanity check
				...
				fallthrough // not ready should mean not enough signatures
