...
// EdCoinThresh keys threshold keys that should be generated using a
...
func (et *EdCoinThresh) CheckCoinProof(msg sig.SignedMessage, prf sig.CoinProof) error {
	return prf.(*coinproof.CoinProof).Validate(et.partPub.edPub.pub, et.sharedPub.pub, msg.GetSignedMessage())
}
...
func (et *EdCoinThresh) CombineProofs(_ sig.Priv, items []*sig.SigItem) (coinVal types.BinVal, err error) {
	...
	for i := 0; i < et.GetT(); i++ {
		shares = append(shares, &share.PubShare{
			...
			V: items[i].Sig.(*coinproof.CoinProof).GetShare(),
		})
	}
