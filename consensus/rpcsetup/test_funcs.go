...
func getAllPubKeys(scs *SingleConsState) error {
	...
	for _, parInfo := range allPar {
		...
		case types.TBLS, types.TBLSDual, types.EDCOIN, types.CoinDual: // TBLS keys are constructed below
		default:
			var pub sig.Pub
...
func initialSetup(setup *SingleConsSetup) (err error) {
	...
	*setup.SetInitialConfig = true
	...
	default:
		scs.PrivKey, err = cons.MakeKey(scs.To)
		if err != nil {
			return
		}
	}
	...
	case types.CoinDual:
		...
		thrsh := cons.GetCoinThresh(scs.To)
		if scs.I < numMembers {
