...
type ParticipantRegister struct {
	...
	dssShared     ed.CoinSharedMarshaled             // For when using EDCOIN threshold signatures, note note safe since centralized
...
func (pr *ParticipantRegister) GenDSSShared(numNonNumbers, numThresh int) error {
	...
	dsss, err := ed.NewCoinShared(pr.count, numNonNumbers, numThresh).PartialMarshal()
	if err != nil {
		return err
	}
	pr.dssShared = dsss
